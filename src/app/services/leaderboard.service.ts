...
export class LeaderboardService {
  ...
  private readonly DB_FOLDER = 'GameData'
  ...
  createObject(obj: any): Observable<any> {
    ...
    return this.http.post(this.FIREBASE_URL + '/' + this.DB_FOLDER + '.json?auth=' + token, obj).pipe(
      switchMap((response: any) => {
        ...
        return this.updateObject(objWithId, obj.id)
      })
    )
  }

  updateObject(obj: any, path: string): Observable<any> {
    const token = this.cookieService.get(this.COOKIE_KEY)
    return this.http.put(this.FIREBASE_URL + '/' + this.DB_FOLDER + '/' + path + '.json?auth=' + token, obj)
  }
